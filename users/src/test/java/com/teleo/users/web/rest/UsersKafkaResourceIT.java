package com.teleo.users.web.rest;
...
import com.teleo.users.IntegrationTest;
import com.teleo.users.config.EmbeddedKafka;
...
class UsersKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/users-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/users-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/users-kafka/unregister"));
                return;
            }
        }
